...
\begin{enumerate}
    ...
    \item \textbf{Avaliação 2: Projeto de desenvolvimento} \\
...
\begin{boxAtividade}[Atividade prática 4 — Luz Acesa e Luz Apagada]
...
\subsection{Aquecimento — Micro Programas com APIs}

Antes do desafio final, vamos praticar com APIs simples que retornam dados divertidos.

\begin{boxAtividade}[Atividade prática 1 — O Oráculo de Conselhos]
\textbf{A Intenção:} Consultar uma API que retorna um conselho aleatório em inglês e exibir na tela.
...
\begin{boxAtividade}[Atividade prática 2 — Buscador de Pets]
\textbf{A Intenção:} Buscar uma imagem aleatória de um cachorro e atualizar o atributo \texttt{src} da tag \texttt{<img>}.
...
\begin{boxAtividade}[A Intenção do Código]
Nesta atividade, vamos conectar as pontas: o usuário digita um CEP no seu \texttt{index.html} $\Rightarrow$ O JavaScript faz uma requisição para o servidor do ViaCEP $\Rightarrow$ O servidor nos responde um pacote de dados (JSON) $\Rightarrow$ Nós "abrimos" esse pacote e usamos o DOM para preencher o endereço na tela automaticamente.
\end{boxAtividade}

\begin{boxAtividade}[Atividade prática 3 — Integração Completa (Fetch + DOM)]
\begin{boxTabela}
\textbf{Arquivos desta Aula:} \texttt{roteiro-8/index.html} \\
\textbf{O que fazer?} Implementar a lógica de busca de CEP dentro da tag \texttt{<script>}.
\end{boxTabela}

Dentro da sua pasta \texttt{roteiro-8/}, abra o arquivo \texttt{index.html} e implemente a lógica de busca:
...
\subsection{Parte Final — Mantendo a Organização}

\begin{boxAtividade}[Atividade prática 4 — Sincronizando seu Trabalho]
...
\section{Roteiro Prático 09 — Avaliação 2: Projeto de desenvolvimento}
...
\begin{itemize}
\item Integrar todos os conceitos abordados durante a Unidade Curricular;
\item Estruturar uma aplicação Single Page Application (SPA) simples;
\item Consumir APIs públicas para preencher conteúdo dinamicamente;
\item Garantir a responsividade e a estilização avançada com Flexbox/Grid.
\end{itemize}
\end{boxObjetivos}

\subsection{Introdução ao Projeto de Desenvolvimento}
...
Sugestões de temas: 
\begin{itemize}
    \item \textbf{Catálogo de Filmes:} Consumindo a OMDb API ou similar.
    \item \textbf{Dashboard de Clima:} Consumindo a OpenWeather API.
    \item \textbf{Pokédex Web:} Consumindo a PokéAPI.
\end{itemize}
\end{boxDica}

\subsection{Blocos de Execução}

O projeto deve ser construído respeitando as etapas de desenvolvimento profissional:

\begin{boxAtividade}[Bloco 1: Estruturação e Semântica (HTML5)]
\textbf{Objetivo:} Criar o "esqueleto" da aplicação.
\begin{itemize}
    \item Crie o arquivo \texttt{index.html}.
    \item Utilize tags semânticas (\texttt{<header>}, \texttt{<main>}, \texttt{<section>}, \texttt{<footer>}).
    \item Adicione um formulário de busca (input text e botão) no topo.
    \item Crie uma área (\texttt{<div id="resultado">}) onde os dados da API aparecerão depois.
\end{itemize}
\end{boxAtividade}

\begin{boxAtividade}[Bloco 2: Layout e Estilização (CSS3)]
\textbf{Objetivo:} Aplicar a identidade visual e o design responsivo.
\begin{itemize}
    \item Crie um arquivo \texttt{style.css} e referencie no HTML.
    \item Utilize \textbf{Flexbox} ou \textbf{CSS Grid} para organizar os elementos da tela.
    \item Adicione estilos para botões e inputs (remova bordas padrão, adicione hover).
    \item Utilize Media Queries (\texttt{@media}) para garantir que a aplicação fique boa tanto em Celulares quanto em Desktop.
\end{itemize}
\end{boxAtividade}

\begin{boxAtividade}[Bloco 3: Interatividade (JS + DOM)]
\textbf{Objetivo:} Escutar as ações do usuário.
\begin{itemize}
    \item Crie o arquivo \texttt{script.js} e anexe ao final do \texttt{<body>}.
    \item Utilize \texttt{document.querySelector} para mapear o formulário e o botão de busca.
    \item Adicione um escutador de eventos (\texttt{addEventListener('click', ...)}) para o botão.
    \item Crie uma função que valide se o usuário digitou algo antes de buscar. Se vazio, exiba um alerta.
\end{itemize}
\end{boxAtividade}

\begin{boxAtividade}[Bloco 4: Integração Externa (Fetch API)]
\textbf{Objetivo:} Dar vida à aplicação com dados reais.
\begin{itemize}
    \item Na função acionada pelo botão (Bloco 3), faça um \texttt{fetch()} para a API escolhida.
    \item Receba a resposta, converta-a via \texttt{.json()}.
    \item Após receber os dados, crie elementos dinâmicos (ex: \texttt{document.createElement('div')} como um "card") para exibir os dados do JSON.
    \item Acrescente atributos e classes dinâmicas ao card (\texttt{classList.add}) e finalmente anexe na tela com \texttt{appendChild}.
\end{itemize}
\end{boxAtividade}

\subsection{Checklist de Entrega Final}

\begin{boxAtividade}[Micro-Commit e Entrega]
\begin{itemize}
    \item Todos os arquivos (\texttt{html}, \texttt{css}, \texttt{js}) estão corretamente separados?
    \item A aplicação não quebra ao ser visualizada em telas pequenas?
    \item Os dados retornam da API de forma consistente?
\end{itemize}
